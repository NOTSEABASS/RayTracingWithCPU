...
public:
    ...
    Triangle(vec3 P1, vec3 P2, vec3 P3, vec3 C, float specRate) 
    { 
        p1 = P1, p2 = P2, p3 = P3; 
        material.normal = normalize(cross(p2 - p1, p3 - p1)); material.color = C; material.specularRate = specRate;
    }
    vec3 p1, p2, p3;    // 三顶点
...
public:
    ...
    Sphere(vec3 o, double r, vec3 c, double specularRate) { O = o; R = r; material.color = c; material.specularRate = specularRate; }
    vec3 O;             // 圆心
    double R;           // 半径
    Material material;  // 材质
...
int main()
{
    ...
    // 球
    Sphere* s1 = new Sphere(vec3(-0.6, -0.8, 0.6), 0.2, WHITE);
    Sphere* s2 = new Sphere(vec3(-0.1, -0.7, 0.2), 0.3, CYAN);
    Sphere* s3 = new Sphere(vec3(0.5, -0.6, -0.5), 0.4, WHITE);
    s1->material.specularRate = 0.3;
    s1->material.roughness = 0.1;
    s1->material.refractRate = 0.2;
    s2->material.specularRate = 0.6;
    s2->material.roughness = 0.1;
    s2->material.refractRate = 0.2;
    s3->material.specularRate = 0.9;
    s3->material.roughness = 0.1;
    s3->material.refractRate = 0.2;
    shapes.push_back(s1);
    shapes.push_back(s2);
    shapes.push_back(s3);

    // 底部平面
    shapes.push_back(new Triangle(vec3(10, -1, 10), vec3(-10, -1, -10), vec3(-10, -1, 10), WHITE, 1.0));
    shapes.push_back(new Triangle(vec3(10, -1, 10), vec3(10, -1, -10), vec3(-10, -1, -10), WHITE, 1.0));

    // 后方平面
    shapes.push_back(new Triangle(vec3(1, 2, -1), vec3(-1, -1, -1), vec3(-1, 2, -1), RED));
    shapes.push_back(new Triangle(vec3(1, 2, -1), vec3(1, -1, -1), vec3(-1, -1, -1), RED));

    
    // 左侧平面
    shapes.push_back(new Triangle(vec3(1, -1, -2), vec3(1, -1, 2), vec3(1, 2, -2), GREEN));
    shapes.push_back(new Triangle(vec3(1, 2, -2), vec3(1, -1, 2), vec3(1, 2, 2), GREEN));

    // 右侧平面
    shapes.push_back(new Triangle(vec3(-1, -1, -2), vec3(-1, -1, 2), vec3(-1, 2, -2), BLUE));
    shapes.push_back(new Triangle(vec3(-1, 2, -2), vec3(-1, -1, 2), vec3(-1, 2, 2), BLUE));
    ...
    // 光源
    Triangle l1 = Triangle(vec3(0.5, 0.99, 0.4), vec3(-0.5, 0.99, -0.4), vec3(-0.5, 0.99, 0.4), WHITE);
    Triangle l2 = Triangle(vec3(0.5, 0.99, 0.4), vec3(0.5, 0.99, -0.4), vec3(-0.5, 0.99, -0.4), WHITE);
